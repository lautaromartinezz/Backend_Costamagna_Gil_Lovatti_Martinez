...
function sanitizeEstablecimientoInput(
  req: Request,
  res: Response,
  next: NextFunction
) {
...
async function findOne(req: Request, res: Response) {
  try {
    ...
    const establecimiento = await em.findOneOrFail(
      Establecimiento,
      { id },
      { populate: ['partidos'] }
    );
    res
      .status(200)
      .json({ message: 'Establecimiento encontrado', data: establecimiento });
  } catch (error: any) {
...
async function add(req: Request, res: Response) {
  try {
    ...
    res
      .status(201)
      .json({ message: 'Establecimiento creado', data: establecimiento });
  } catch (error: any) {
...
async function update(req: Request, res: Response) {
  try {
    const id = Number.parseInt(req.params.id);
    const establecimientoToUpdate = await em.findOneOrFail(Establecimiento, {
      id,
    });
...
async function remove(req: Request, res: Response) {
  try {
    const id = Number.parseInt(req.params.id);
    const establecimientoToRemove = await em.findOneOrFail(Establecimiento, {
      id,
    });
